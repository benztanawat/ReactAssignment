...
describe("searchUser", () => {
  it("should return listLeaderBoardData as similar as searchName and order DESC by rank", () => {
    const searchUserData = searchUser("vid");
    expect(searchUserData).toStrictEqual([
      {
        bananas: 3550,
  ...
  it("should return listLeaderBoardData filtered by searchName, ordered in descending order by rank, and inser at the first index if searchName matches a name in the data", () => {
    const searchUserData = searchUser("David");
    expect(searchUserData[0]).toStrictEqual({
      bananas: 3150,
      lastDayPlayed: "2018-12-27",
      longestStreak: 1,
      name: "David",
